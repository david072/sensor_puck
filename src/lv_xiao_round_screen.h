...
Arduino_DataBus* bus =
    new Arduino_ESP32SPI(XIAO_DC, XIAO_CS, SCK, MOSI, MISO, FSPI);
...
Arduino_DataBus* bus =
    new Arduino_RPiPicoSPI(XIAO_DC, XIAO_CS, SCK, MOSI, MISO, spi0);
#else
...
void xiao_disp_flush(lv_display_t* disp, lv_area_t const* area, uint8_t* px_map)
#elif LVGL_VERSION_MAJOR == 8
void xiao_disp_flush(lv_disp_drv_t* disp, lv_area_t const* area,
                     lv_color_t* color_p)
#else
